...
<body>
    ...
    <div class="container my-5">
        ...
        <div class="display1 row my-3">
            ...
            <ul id="pagination">
            <!-- 맨 첫 페이지로 -->
                <li><a href="${ path }/board/notice_search?page=1&searchType=${searchType}&keyword=${keyword}">«</a></li>
			
			<!-- 이전 페이지로 -->
				<li><a href="${ path }/board/notice_search?page=${ pageInfo.prevPage }&searchType=${searchType}&keyword=${keyword}">‹</a></li>    
            ...
            <!--  10개 페이지 목록 -->
				<c:forEach begin="${ pageInfo.startPage }" end="${ pageInfo.endPage }" varStatus="status">
					...
					<c:if test="${ status.current != pageInfo.currentPage }">
                		<li><a href="${ path }/board/notice_search?page=${ status.current }&searchType=${searchType}&keyword=${keyword}">${ status.current }</a></li>
					</c:if>
				</c:forEach>
				
			<!-- 다음 페이지로 -->
				<li><a href="${ path }/board/notice_search?page=${ pageInfo.nextPage }&searchType=${searchType}&keyword=${keyword}">›</a></li>  
			<!-- 맨 끝 페이지로 -->
                <li><a href="${ path }/board/notice_search?page=${ pageInfo.maxPage }&searchType=${searchType}&keyword=${keyword}">»</a></li>
            </ul>
