                ...
                <div class="container-fluid">
                  ...
                  <div class="card shadow mb-4 col-12">
                      <div class="card-header py-3">
                          <h6 class="m-0 font-weight-bold text-primary" style="line-height: 3rem;">매출액
                           <c:set var="no" value="no" />
                            <select class="form-control" style="width: 15%; float: right;" onchange="if(this.value) location.href=(this.value);">
                               <option value="${path}/admin/admin_total_sales?no=00"selected="selected">월별</option>
							 <c:forEach var="monthlydata" items="${monthlydata}" varStatus="i">
                               <option value="${path}/admin/admin_total_sales?no=${monthlydata.monthdata}">${monthlydata.monthdata}</option>
                             </c:forEach>
                            </select>
                          </h6>
                      </div>
                              ...
                              <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                                  ...
                                  <c:if test="${ empty list }">
                                    ...
                                    <c:forEach var="Totalview" items="${ list }"> 
                                      <tr>
                                          <td id="td"><fmt:formatDate value="${Totalview.salesDate}" type="date"></fmt:formatDate></td>
                                          <td id="td"><fmt:formatNumber type="number" value="${Totalview.salesCount}" groupingUsed="true"/></td> 
                                          <c:set var="Total" value="${Total + Totalview.salesCount }" />
                                      </tr>
                                    </c:forEach>
